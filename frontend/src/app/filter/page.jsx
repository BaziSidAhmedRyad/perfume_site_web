...
import {filteredPerfumes} from "../../_lib/recuperation"
  ...
  Setfilter(prev => {
    ...
    return {
      ...
      [key]: current.includes(value)
        ? current.filter(v => v !== value) // si déjà sélectionné → enlever
        : [...current, value]              // sinon → ajouter
    };
  });
};

const rechercher=()=>{
const resultat=filteredPerfumes(Parfumes,filter);
if(resultat){
    dispatch(Setproduct(resultat));
}
}

    return(
        <div className="flex flex-col  ml-[43px] md:ml-[90px] gap-[200px] h-auto md:h-[1100px] bg-gray-100 rounded-[20px] md:w-[20%] w-[76%] p-[60px]">
                          ...
                          { categories.map((item,index)=>
                           (
                            <div key={index} className="flex gap-[10px]">
                                <input type='checkbox' checked={filter.categories === item} onChange={() => handchange("categories", item)}  />
                    ...
                    { opentwo&&(
                           <div className="absolute top-[40px] left-[-10px] flex flex-col w-[200px] p-[10px] gap-[10px] bg-white rounded-[20px]">
                            {weather.map((item,index)=>(
                             <div key={index} className="flex gap-[10px]">
                                <input type='checkbox' checked={filter.weather === item} onChange={() => handchange("weather", item)} />
                              <p className="font-semibold">{item}</p>
                             </div>
                            ))
                            }
                    ...
                    { openthree &&(
                        <div className="absolute top-[40px] left-[-10px] flex flex-col w-[200px] p-[10px] gap-[10px] bg-white rounded-[20px]">
                            {season.map((item,index)=>(
                             <div key={index} className="flex gap-[10px] ">
                                <input type='checkbox' checked={filter.season === item} onChange={() => handchange("season", item)} />
                                  <p className="font-semibold ">{item}</p>
                             </div>
                            ))
                            }
                    ...
                    <div className="absolute top-[40px] left-[-10px] flex flex-col w-[200px] p-[10px] gap-[10px] bg-white rounded-[20px]">
                        {
                            gouts.map((item,index)=>(
                                <div key={index} className="flex  gap-[10px]">
                                    <input type='checkbox' className="rounded-[10px]" checked={filter.gouts === item} onChange={() => handchange("gouts", item)} />
                                    <p className="font-semibold">{item}</p>
                                </div>
                            ))
                        }
            ...
            <div className="block md:hidden">
               <input type='button' value='rechercher' onClick={()=>rechercher()} className="rounded-[10px] bg-gray-100 text-gray-900 p-[12px] border-2 border-gray-300 hover:bg-white  w-[187px] text-[23px] font-bold " />
            </div>
        </div>
    )
}